...
#  redis（https://heuqqdmbyk.feishu.cn/wiki/O4vwwWkEXiYEPyk8EpFcqvmLnth）
...
>  **新生代回收（Minor GC，复制算法）**
>
> 1. **对象分配**：新对象优先分配至**Eden区**。
> 2. **触发条件**：当Eden区满时，触发Minor GC。
> 3. **存活对象复制**：
>    - Eden区和现在`From Survivor`中年龄未达阈值的存活对象被复制到`To Survivor`。
> 4. **年龄计数**：每经历一次Minor GC，存活对象的年龄加1。
> 5. **晋升老年代**：当对象年龄达到阈值（默认15），或Survivor区空间不足时，对象晋升至老年代。
> 6. **交换Survivor区**：每次GC后，`From`和`To`角色互换（`From`变`To`, `To`变`From`），确保`To`区始终为空。
>
> **老年代回收（Major GC/Full GC，标记-清除/整理算法）**
>
> 

![image-20250417092417374](assets/images/image-20250417092417374.png)

![image-20250417092852880](assets/images/image-20250417092852880.png)
...
1. **内存分区（Region）**
   - 将堆划分为**固定大小的Region**（1MB~32MB），支持动态分配为Eden、Survivor、Old或Humongous区37。
   - **Humongous区**：专用于存储超过Region容量50%的大对象，若需连续存储多个Region可能触发Full GC59。
2. **分代与逻辑隔离**
   - 逻辑上保留分代（年轻代、老年代），但物理上不要求连续。年轻代占比动态调整（默认初始5%~最大60%）38。
3. **可预测的停顿时间模型**
...
1. **初始标记（Initial Mark）**
   - **STW**阶段，标记GC Roots直接关联的对象，耗时短37。
2. **并发标记（Concurrent Mark）**
   - 遍历对象图标记存活对象，与用户线程并行，可能产生**浮动垃圾**69。
3. **最终标记（Final Mark）**
   - **STW**修正并发标记期间引用变化，使用**SATB（原始快照）**解决漏标问题57。
4. **筛选回收（Evacuation）**
   - 根据Region回收价值排序，选择部分Region复制存活对象到空闲Region，实现局部压缩19。

**触发条件**：

- **Young GC**：Eden区满时触发，复制存活对象至Survivor或晋升Old区38。
- **Mixed GC**：老年代占用达阈值（默认45%，`-XX:InitiatingHeapOccupancyPercent`），回收年轻代+部分老年代Region79。
